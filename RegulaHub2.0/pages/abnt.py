...
def main():
    ...
    # Depuração: Verifica o e-mail do usuário logado
    st.write(f"Debug: Usuário logado com e-mail: {st.session_state.get('email', 'Nenhum')}")

    st.title("Regulahub2.0 - ABNT")
    st.write("Gerencie documentos regulatórios da ABNT.")

    orgao = "ABNT"
    subtitulos = ORGAOS_TITULOS.get(orgao, [])
    user_is_admin = is_admin(st.session_state.email)

    # Depuração: Verifica se o usuário é administrador
    if user_is_admin:
        st.write("Debug: Usuário é administrador")
    else:
        st.write("Debug: Usuário NÃO é administrador")
        ...
        if pdfs:
            for pdf_id, filename, title, upload_date, topic, subtopic in pdfs:
                ...
                if col1.button(
                    ...
                    key=f"view_pdf_{orgao}_{subtopic.replace(' ', '_').replace('/', '_')}_{pdf_id}"  # Sanitiza subtopic
                ):
                    st.session_state.selected_pdf_id = pdf_id
                    st.session_state.selected_orgao = orgao
                    st.session_state.selected_subtitulo = subtopic
                    st.rerun()
                if user_is_admin and col2.button("Excluir", key=f"delete_pdf_{orgao}_{subtopic.replace(' ', '_').replace('/', '_')}_{pdf_id}"):
                    try:
                        if delete_pdf(pdf_id):
                            st.success(f"PDF '{display_title}' excluído com sucesso!")
                            st.rerun()
                        else:
                            st.error("Falha ao excluir o PDF: ID não encontrado ou erro no banco.")
            ...
            try:
                ...
                if result:
                    ...
                    st.download_button(
                        ...
                        key=f"download_{orgao}_{subtitulo.replace(' ', '_').replace('/', '_')}_{pdf_id}"
                    )
                    if user_is_admin and st.button("Excluir PDF", key=f"delete_pdf_main_{orgao}_{subtitulo.replace(' ', '_').replace('/', '_')}_{pdf_id}"):
                        try:
                            if delete_pdf(pdf_id):
                    ...
                    st.error("PDF não encontrado no banco de dados.")
            except Exception as e:
                st.error(f"Erro ao acessar PDF: {str(e)}")
        else:
